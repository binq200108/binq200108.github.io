(function () {
  ...
  function setTheme(theme) {
    var normalizedTheme = applyTheme(theme);
  ...
  if (window.matchMedia && window.matchMedia('(pointer: coarse)').matches) {
    ...
    document.addEventListener('touchend', function (e) {
      ...
      if (
        ...
        target &&
        target.closest &&
        !target.closest(doubleTapBypassSelector)
      ) {
        e.preventDefault();
      }
      lastTouchEndAt = now;
  ...
  /* ========== Live Photos (desktop: click toggle, mobile: long-press) ========== */
  (function initLivePhotos() {
    ...
    cards.forEach(function (card, idx) {
      ...
      /* 移动端：长按播放，松手停止（Apple Live Photo 风格） */
      if (coarsePointer) {
        var lpTimer = null;
        var lpActive = false;
        var lpConsumed = false;
        var lpStartX = 0;
        var lpStartY = 0;
        var LP_DELAY = 250;
        var LP_MOVE = 10;
        ...
        card.addEventListener('touchmove', function (e) {
          ...
          var touch = e.touches[0];
          var dx = Math.abs(touch.clientX - lpStartX);
          var dy = Math.abs(touch.clientY - lpStartY);
          if (dx > LP_MOVE || dy > LP_MOVE) {
        ...
        /* 长按结束后抑制紧随的 click，防止误触发 Lightbox */
  ...
  (function initLightbox() {
    ...
    if (!lightboxBackdrop) {
      ...
      lightboxBackdrop.id = 'lightbox-backdrop';
      lightboxBackdrop.setAttribute('aria-hidden', 'true');
      lightbox.insertBefore(lightboxBackdrop, lightbox.firstChild);
    }
    ...
    var dismissDragOffsetY = 0;
    var dismissDragScale = 1;
    var dismissDragOverlayOpacity = 1;
    var bodyScrollLocked = false;
    var bodyOverflowBeforeLock = '';
    var bodyPaddingRightBeforeLock = '';
    var hiddenFlipSource = null;
    var hiddenFlipSourceVisibility = '';
    var activeAnims = [];

    var ZOOM_FACTOR = 1.2;
    var MIN_ZOOM = 1;
    var MAX_ZOOM = 16;
    var SWIPE_BUFFER_PREPARE_PX = 14;
    ...
    var connectionInfo = navigator.connection || navigator.mozConnection || navigator.webkitConnection || null;
    ...
    // Use PhotoSwipe-like thumbnail-to-viewer motion unless explicitly disabled.
    var stableOpenMode = flipMotionConfig.stableOpenMode === true;
    var closeFlipMode = flipMotionConfig.closeFlipMode !== false;
    var lightboxLiteMotion = shouldUseLiteMotion();
    lightbox.classList.toggle('lite-motion', lightboxLiteMotion);
    lightbox.classList.toggle('stable-open', stableOpenMode);
    ...
    function shouldUseLiteMotion() {
      if (reducedMotion && reducedMotion.matches) return true;
      if (flipMotionConfig.liteOnCoarse === true && coarseMatcher && coarseMatcher.matches) return true;
      if (connectionInfo && connectionInfo.saveData) return true;

      var deviceMemory = navigator.deviceMemory || 0;
      var hardwareConcurrency = navigator.hardwareConcurrency || 0;
      if (flipMotionConfig.liteOnLowPower === true && deviceMemory && deviceMemory <= 4) return true;
      if (flipMotionConfig.liteOnLowPower === true && hardwareConcurrency && hardwareConcurrency <= 4) return true;
      return false;
    }
    ...
    function setMotionPhase(phase) {
      lightbox.classList.toggle('lite-motion', lightboxLiteMotion);
      lightbox.classList.toggle('stable-open', stableOpenMode);
      lightbox.classList.toggle('is-opening', phase === 'opening');
      lightbox.classList.toggle('is-closing', phase === 'closing');
      lightbox.classList.toggle('is-animating', phase === 'opening' || phase === 'closing');
    }

    function getLightboxViewportBounds() {
      var compact = window.innerWidth <= 768;
      return {
        maxW: window.innerWidth * (compact ? 0.96 : 0.9),
        maxH: window.innerHeight * (compact ? 0.85 : 0.9)
      };
    }

    function clearExifRevealTimer() {
      if (!exifRevealTimer) return;
      clearTimeout(exifRevealTimer);
      exifRevealTimer = null;
    }

    function hideExifImmediate() {
      if (!lbExif) return;
      clearExifRevealTimer();
      lbExif.classList.remove('visible', 'fade-out');
      lbExif.innerHTML = '';
      lbExif.removeAttribute('style');
    }

    function revealExifAfterOpen(sourceImg) {
      if (!lbExif || !sourceImg) return;
      clearExifRevealTimer();
      exifRevealTimer = setTimeout(function () {
        exifRevealTimer = null;
        if (!lightbox.classList.contains('active')) return;
        if (lightbox.classList.contains('is-animating')) return;
        updateExif(sourceImg, { immediate: lightboxLiteMotion });
      }, lightboxLiteMotion ? 0 : 90);
    }
    ...
    function computeFitScale() {
      ...
      var bounds = getLightboxViewportBounds();
      fitScale = Math.min(bounds.maxW / effective.width, bounds.maxH / effective.height, 1);
    }
    ...
    function setUiHidden(hidden) {
      ...
      if (!uiHidden && !lightbox.classList.contains('is-animating')) {
    ...
    function preloadAdjacent(index) {
      ...
      preloadImage(getImageDisplaySrc(images[prev]));
      preloadImage(getImageDisplaySrc(images[next]));
    }
    ...
    function buildCenteredRect(width, height) {
      ...
      var bounds = getLightboxViewportBounds();
      var scale = Math.min(bounds.maxW / w, bounds.maxH / h, 1);
    ...
    function hideSourceForFlip(sourceImg) {
      restoreSourceAfterFlip();
      if (!sourceImg) return;
      hiddenFlipSource = sourceImg;
      hiddenFlipSourceVisibility = sourceImg.style.visibility || '';
      sourceImg.style.visibility = 'hidden';
    }

    function restoreSourceAfterFlip() {
      if (!hiddenFlipSource) return;
      hiddenFlipSource.style.visibility = hiddenFlipSourceVisibility;
      hiddenFlipSource = null;
      hiddenFlipSourceVisibility = '';
    }
    ...
    function playOverlayFade(fromOpacity, toOpacity, duration, easing, delay) {
      ...
      function cleanup() {
        ...
        cancelAnimationFrame(raf2);
        clearTimeout(timer);
        if (lightboxBackdrop) lightboxBackdrop.removeEventListener('transitionend', onEnd);
      }
      ...
      var target = lightboxBackdrop || lightbox;
      target.addEventListener('transitionend', onEnd);
      target.style.transition = 'none';
      target.style.opacity = String(fromOpacity);
      raf1 = requestAnimationFrame(function () {
        raf2 = requestAnimationFrame(function () {
          if (done) return;
          target.style.transition = 'opacity ' + duration + 'ms ' + easing + ' ' + safeDelay + 'ms';
          target.style.opacity = String(toOpacity);
    ...
    function updateExif(sourceImg, options) {
      if (!lbExif) return;
      options = options || {};
      ...
      if (wasVisible && alreadyFading && !immediate) {
        ...
        // Fresh/immediate show: place content and reveal with optional fade.
        lbExif.innerHTML = '';
        lbExif.appendChild(newContent);
        lbExif.classList.add('visible');
        lbExif.classList.remove('fade-out');
        lbExif.style.transition = 'none';
        lbExif.style.opacity = immediate ? '1' : '0';
        positionExifNearImage();
        void lbExif.offsetWidth;
        lbExif.style.transition = '';
    ...
    function cancelActiveAnims() {
      ...
      activeAnims = [];
      restoreSourceAfterFlip();
      lightbox.classList.remove('flip-opening');
      setMotionPhase('idle');
    }
    ...
    function openLightbox() {
      ...
      var disableFlipOnCoarse = flipMotionConfig.disableFlipOnCoarse === true && !!(coarseMatcher && coarseMatcher.matches);
      var canFlipOpen = !stableOpenMode && isUsableRect(thumbRect) && !!thumbSrc && !disableFlipOnCoarse && !lightboxLiteMotion;
      var mainFadeDuration = stableOpenMode ? 0 : (lightboxLiteMotion ? 70 : 90);
      var overlayOpenDuration = stableOpenMode ? 0 : (lightboxLiteMotion ? 120 : flipMotion.overlayOpenDuration);
      var overlayOpenDelay = stableOpenMode ? 0 : (lightboxLiteMotion ? 0 : flipMotion.overlayOpenDelay);
      var imageReady = false;
      var flipReady = !canFlipOpen;
      var overlayReady = false;
      var openFinalized = false;
      var preparedFlipOpen = false;
      ...
      function settleOpenUi() {
        if (!openFinalized) return;
        if (!overlayReady && !lightboxLiteMotion) return;
        setMotionPhase('idle');
        revealExifAfterOpen(img);
      }
      ...
      function maybeFinalizeOpen() {
        ...
        if (canFlipOpen) {
          ...
          setMainImageOpacity(1, mainFadeDuration, 'cubic-bezier(0.2, 0.8, 0.2, 1)');
          hideFlipGhost();
          restoreSourceAfterFlip();
        }
      ...
      if (canFlipOpen) {
        ...
        if (showFlipGhost(img, thumbSrc, thumbRect, targetRect)) {
          preparedFlipOpen = true;
          lightbox.classList.add('flip-opening');
          hideSourceForFlip(img);
        } else {
          flipReady = true;
        }
      }
      ...
      lightbox.style.opacity = '1';
      if (lightboxBackdrop) {
        lightboxBackdrop.style.opacity = overlayOpenDuration > 0 ? '0' : '1';
      }
      if (!canFlipOpen) {
        setMainImageOpacity(1, 0);
      } else {
      ...
      if (overlayOpenDuration > 0) {
        var overlayOpen = playOverlayFade(0, 1, overlayOpenDuration, flipMotion.openEasing, overlayOpenDelay);
        if (overlayOpen && overlayOpen.finished) {
          overlayOpen.finished.then(function () {
            if (lightboxBackdrop) lightboxBackdrop.style.opacity = '1';
            overlayReady = true;
            settleOpenUi();
          }).catch(function () {});
        } else {
    ...
    function closeLightbox() {
      ...
      var r = ((rotation % 360) + 360) % 360;
      var disableFlipOnCoarse = flipMotionConfig.disableFlipOnCoarse === true && !!(coarseMatcher && coarseMatcher.matches);
      var canFlipBack = closeFlipMode && isUsableRect(thumbRect) && !isZoomed && r === 0 && !disableFlipOnCoarse && !lightboxLiteMotion;
      var overlayCloseDuration = lightboxLiteMotion ? 110 : flipMotion.overlayCloseDuration;
      var overlayCloseDelay = lightboxLiteMotion ? 0 : flipMotion.overlayCloseDelay;
      ...
      if (!canFlipBack) {
        setMainImageOpacity(0, overlayCloseDuration, flipMotion.closeEasing);
        var fadeOnlyAnim = playOverlayFade(1, 0, overlayCloseDuration, flipMotion.closeEasing, overlayCloseDelay);
      ...
      if (!isUsableRect(fromRect)) {
        setMainImageOpacity(0, overlayCloseDuration, flipMotion.closeEasing);
        var fadeAnim = playOverlayFade(1, 0, overlayCloseDuration, flipMotion.closeEasing, overlayCloseDelay);
      ...
      if (!closeSrc || !showFlipGhost(activeImage, closeSrc, fromRect, thumbRect)) {
        setMainImageOpacity(0, overlayCloseDuration, flipMotion.closeEasing);
        var fallbackAnim = playOverlayFade(1, 0, overlayCloseDuration, flipMotion.closeEasing, overlayCloseDelay);
      ...
      hideSourceForFlip(activeImage);
      setMainImageOpacity(0, 0);
      var ghostCloseAnim = playFlipGhost(flipMotion.closeDuration, flipMotion.closeEasing);
      var overlayCloseAnim = playOverlayFade(1, 0, overlayCloseDuration, flipMotion.closeEasing, overlayCloseDelay);
      if (overlayCloseAnim && overlayCloseAnim.finished) {
        overlayCloseAnim.finished.then(function () {
          if (lightboxBackdrop) lightboxBackdrop.style.opacity = '0';
        }).catch(function () {});
      }
      ...
      if (ghostCloseAnim && ghostCloseAnim.finished && overlayCloseAnim && overlayCloseAnim.finished) {
        ...
        setTimeout(finishClose, Math.max(flipMotion.closeDuration, overlayCloseDuration + overlayCloseDelay) + 120);
      }
    }
    ...
    function applyDismissDrag(dy) {
      ...
      var safeOpacity = clamp(overlayOpacity, 0.35, 1);

      dismissDragOffsetY = safeDy;
      dismissDragScale = scale;
      dismissDragOverlayOpacity = safeOpacity;

      lightbox.style.transition = 'none';
      lightbox.style.opacity = String(safeOpacity);
      lightboxImg.style.transition = 'none';
    ...
    function resetDismissDragVisual() {
      ...
      dismissDragScale = zoomScale;
      dismissDragOverlayOpacity = 1;
      setTimeout(function () {
        if (!lightbox.classList.contains('active')) return;
        lightboxImg.style.transition = '';
    ...
    function closeLightboxFromDismiss(dy) {
      if (singleTapTimer) {
        clearTimeout(singleTapTimer);
        singleTapTimer = null;
      }
      ...
      var safeDy = dismissDragOffsetY || dy || 0;
      var direction = safeDy >= 0 ? 1 : -1;
      var extraTravel = Math.max(window.innerHeight * 0.18, 120);
      var targetY = safeDy + direction * extraTravel;
      var currentScale = dismissDragScale || 1;
      var targetScale = Math.max(0.78, currentScale - 0.06);
      var startOpacity = clamp(dismissDragOverlayOpacity || 1, 0, 1);
      var duration = lightboxLiteMotion ? 140 : 180;
      var finished = false;
      ...
      lightbox.style.transition = 'opacity ' + duration + 'ms cubic-bezier(0.2, 0.8, 0.2, 1)';
      lightbox.style.opacity = String(startOpacity);
      lightboxImg.style.transition = 'transform ' + duration + 'ms cubic-bezier(0.2, 0.8, 0.2, 1), opacity ' + duration + 'ms ease';
      lightboxImg.style.transform = 'translate3d(0,' + safeDy + 'px,0) rotate(' + rotation + 'deg) scale(' + currentScale.toFixed(4) + ')';
      lightboxImg.style.opacity = '1';
      ...
      requestAnimationFrame(function () {
        requestAnimationFrame(function () {
          ...
          lightboxImg.style.transform = 'translate3d(0,' + targetY.toFixed(2) + 'px,0) rotate(' + rotation + 'deg) scale(' + targetScale.toFixed(4) + ')';
          lightboxImg.style.opacity = '0.92';
        });
      });

      setTimeout(finishClose, duration + 80);
    }
    ...
    lightboxImg.addEventListener('touchmove', function (e) {
      ...
      if (touchMode === 'swipe' && !isZoomed) {
        ...
        if (Math.abs(dx) > SWIPE_BUFFER_PREPARE_PX && lightboxImgBuffer) {
          ...
          if (!interactiveSwipeActive || currentDir !== interactiveSwipeDir) {
            ...
            if (lightboxImgBuffer.src !== previewSrc) {
    ...
    lightboxImg.addEventListener('touchend', function (e) {
      ...
      if (dismissDragActive) {
        ...
        if (dy > closeThreshold) {
          closeLightboxFromDismiss(rawDy);
        } else {
  ...
  (function () {
    ...
    function usesMobileSelectionUI() {
    ...
    function triggerGitalkLoad() {
      if (typeof window.CG_GITALK_BOOT === 'function') {
        window.CG_GITALK_BOOT('selection-quote');
      }
    }

    function retryFillGitalkTextarea(quotedText, maxAttempts, delay) {
      var attempts = 0;

      function run() {
        attempts += 1;
        if (fillGitalkTextarea(quotedText)) return;
    ...
    quoteBtn.addEventListener('click', function (e) {
      ...
      // Try for a while in case Gitalk is lazy-loaded and still rendering.
      setTimeout(function () {
        retryFillGitalkTextarea(quotedText, 8, 400);
      }, 300);
    });

    // PC: mouseup on post content
    postContent.addEventListener('mouseup', function () {
      if (usesMobileSelectionUI()) return;
      // Small delay to let selection finalize
      setTimeout(showTooltip, 10);
    });

    // Mobile: delay the quote button and dock it to the bottom
    // so it doesn't overlap with the native copy/lookup callout.
  ...
  /* ========== Inline Notes (hover on desktop, tap on mobile) ========== */
  (function initInlineNotes() {
    var notes = Array.prototype.slice.call(document.querySelectorAll('.markdown-body .cg-inline-note'));
    if (!notes.length) return;

    var coarseQuery = window.matchMedia ? window.matchMedia('(pointer: coarse)') : null;
    var isCoarsePointer = !!(coarseQuery && coarseQuery.matches);
    var positionFrame = 0;

    function clearPosition(note) {
      var popover = note.querySelector('.cg-inline-note-popover');
      if (!popover) return;
      popover.classList.remove('is-positioned');
      popover.removeAttribute('data-placement');
      popover.style.left = '';
      popover.style.top = '';
      popover.style.setProperty('--cg-note-arrow-left', '');
    }

    function positionPopover(note) {
      if (isCoarsePointer || !note) return;

      var trigger = note.querySelector('.cg-inline-note-trigger');
      var popover = note.querySelector('.cg-inline-note-popover');
      ...
      var gap = 14;
      var arrowPadding = 20;
      var contentHost = note.closest('.post-content, .markdown-body, .container');
      ...
      var height = popoverRect.height;
      var boundsRect = contentHost ? contentHost.getBoundingClientRect() : null;

      var left = triggerRect.left + triggerRect.width / 2 - width / 2;
      var minLeft = viewportPadding;
      var maxLeft = Math.max(viewportPadding, window.innerWidth - viewportPadding - width);

      if (boundsRect) {
        var boundedMinLeft = Math.max(viewportPadding, boundsRect.left + contentPadding);
        var boundedMaxLeft = Math.min(window.innerWidth - viewportPadding - width, boundsRect.right - contentPadding - width);
        if (boundedMaxLeft >= boundedMinLeft) {
          minLeft = boundedMinLeft;
      ...
      if (placement === 'bottom') {
        ...
        top = Math.min(top, Math.max(viewportPadding, window.innerHeight - viewportPadding - height));
      } else {
      ...
      arrowLeft = Math.max(arrowPadding, Math.min(width - arrowPadding, arrowLeft));

      popover.setAttribute('data-placement', placement);
      popover.style.left = left + 'px';
      popover.style.top = top + 'px';
      popover.style.setProperty('--cg-note-arrow-left', arrowLeft + 'px');
    }

    function schedulePosition(note) {
      if (isCoarsePointer || !note) return;
      window.requestAnimationFrame(function () {
        positionPopover(note);
      });
    }
    ...
    function updateActivePositions() {
      ...
      notes.forEach(function (note) {
        if (note.classList.contains('is-open') || note.matches(':hover') || note.matches(':focus-within')) {
          positionPopover(note);
        }
      });
    }

    function scheduleActivePositions() {
      if (isCoarsePointer || positionFrame) return;
      positionFrame = window.requestAnimationFrame(updateActivePositions);
    }

    function syncPointerMode(e) {
      isCoarsePointer = !!(e && e.matches);
      notes.forEach(function (note) {
    ...
    if (coarseQuery) {
      if (typeof coarseQuery.addEventListener === 'function') {
        ...
        coarseQuery.addListener(syncPointerMode);
      }
    }
    ...
    document.addEventListener('mousedown', function (e) {
      if (e.target && e.target.closest('.cg-inline-note')) return;
      closeAll();
    });
    ...
    window.addEventListener('resize', scheduleActivePositions);
    window.addEventListener('scroll', scheduleActivePositions, { passive: true });
  })();
